...
- `ac online` shows who is online when the SOAP account can run the online-list command, then falls back to connected player counts.
- `ac check` tests whether the configured SOAP host and port are reachable.
- `ac createuser` creates a new account, but only for approved Discord roles.

## Setup

1. Configure the SOAP endpoint the cog should use with `ac set soap_url <ip:port>`.
2. (Optional) Configure SOAP basic auth using `ac set soap_auth <user> <pass>`.
3. Check the current configuration with `ac set view`, then test connectivity with `ac check`.
4. Add display text with `ac set servername`, `ac set realmlist`, and `ac set infodescription`.
5. Add the Discord roles that are allowed to create accounts with `ac set accountcreationrole`.
6. (Optional) Adjust advanced SOAP behavior with `ac set timeout`, `ac set onlinecommand`, `ac set infocommand`, or `ac set createcommand`.

### Helpful examples

- `ac set realmlist wow.sickgaming.net`
- `ac set servername My Realm`
- `ac set soap_url 192.168.86.139:17878`
- `ac set soap_auth soapuser abcd1234`
- `ac set timeout 10`
- `ac set onlinecommand account onlinelist`

If your server exposes SOAP on a non-default host/port, update the SOAP URL accordingly.

`ac info` pulls technical status from the `server info` SOAP command. AzerothCore may not return a friendly realm name or public description through SOAP, so use these settings for display:

```text
ac set servername <name>
ac set infodescription <description>
ac set realmlist <text>
```

`ac online` uses the configured server name as the embed title. If no server name is set, it uses `World of Warcraft`. The configured info description and realmlist are included when available, followed by an `Online Players` section and a small bundled banner image.
...
If `ac info` or `ac online` times out, check that the configured IP/hostname is reachable from the Discord bot host, the SOAP port is published or available on the Docker network, and SOAP is enabled in AzerothCore.
Use `ac check` to test the configured host and port from the bot before troubleshooting SOAP credentials or command permissions.

## Troubleshooting

`ac check` only tests whether the configured host and port accept a TCP connection. A passing TCP check does not prove that AzerothCore SOAP accepted the request. Use `ac info` or `ac online` to test the actual SOAP command path.

Common results:

- `SOAP URL has not been configured yet`: set the endpoint with `ac set soap_url <ip:port>`.
...
- `HTTP 401` or auth-related SOAP fault: check `ac set soap_auth <user> <pass>`.
...
- `Command 'players' does not exist`: reset the old command with `ac set onlinecommand reset`, or configure another command that exists on your AzerothCore build.

`ac online` defaults to `account onlinelist` so it can show who is online instead of only counts. AzerothCore documents this as a higher-security GM command, so the SOAP account may need elevated access or the command security may need to be adjusted in AzerothCore's command table. If the online-list command cannot run or does not return names, `ac online` falls back to `server info` counts when possible.
...
```text
ac set onlinecommand reset
ac set infocommand reset
ac set createcommand reset
```
...
Security note: SOAP credentials are sensitive. Prefer `ac set soap_auth` over embedding passwords in URLs or shell history.
