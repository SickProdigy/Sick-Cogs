...
class Memeify(commands.Cog):
    ...
    @commands.group(name="memeify", aliases=["imgflip"], invoke_without_command=True)
    @commands.bot_has_permissions(attach_files=True)
    async def memeify(self, ctx, template: Optional[str] = None, *, text: str = ""):
        """Create a meme: `memeify <template> <top> | <bottom>`."""
        if template is None:
            await ctx.send_help()
            return
    ...
    @memeify.command(name="list", aliases=["templates"])
    async def memeify_list(self, ctx, *, search: str = ""):
    ...
    @memeify.command(name="avatar")
    @commands.bot_has_permissions(attach_files=True)
    async def memeify_avatar(self, ctx, member: Optional[discord.Member] = None, *, text: str):
    ...
    @memeify.command(name="image", aliases=["attachment", "photo"])
    @commands.bot_has_permissions(attach_files=True)
    async def memeify_image(self, ctx, *, text: str):
        """Caption an attached image: `memeify image <top> | <bottom>`."""
        if not ctx.message.attachments:
            await ctx.send("Attach an image to the command message.")
            return
        attachment = ctx.message.attachments[0]
        if attachment.size > MAX_INPUT_BYTES:
            await ctx.send("The source image is too large.")
            return
        top, bottom = self._captions(text)
        source = await attachment.read()
        async with ctx.typing():
            await self._send_render(ctx, source, top, bottom, "memeified.png", add_bands=True)
