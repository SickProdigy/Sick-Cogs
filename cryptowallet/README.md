...
The five Red fields do not all come from the same screen:
...
| `jwt_kid` | Key ID placed in our custom-auth JWT header and matching public JWK. It tells CDP which SickGaming public key verifies a user token. | Generated by the CryptoWallet JWT/JWKS setup; it is not copied from the API Keys page. | No |
...
4. Complete this cog's custom-auth JWT/JWKS setup. It will create the signing key and `jwt_kid`.
5. Publish the HTTPS JWKS endpoint and configure its URL, expected issuer, audience, and default
   `sub` user identifier under the project's CDP custom-auth settings.
6. As the Red bot owner, run `[p]set api`, set the service to `cryptowallet_cdp`, and enter:
   ...
   jwt_kid YOUR_GENERATED_JWT_KEY_ID
   ```

7. Run `[p]walletset cdpstatus`, then test `[p]wallet` using Base Sepolia only.

Step 4 is the next implementation checkpoint. Until it exists, a fresh installer cannot complete
the intended user-owned custom-auth flow merely by creating Coinbase API credentials.
