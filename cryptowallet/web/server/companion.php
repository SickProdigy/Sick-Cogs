...
function sickwallet_forward_browser_response(string $path): never
{
    if (!preg_match('#^/(?:session/[A-Za-z0-9_-]{32,128}|oauth/callback\\?code=[^&]{1,12288}&state=[A-Za-z0-9_-]{32,128})$#D', $path)) {
        throw new RuntimeException('Unsupported browser proxy path.');
    }
    $responseHeaders = [];
    $handle = curl_init(sickwallet_backend_url() . $path);
    curl_setopt_array($handle, [
    ...
    $error = curl_error($handle);
    curl_close($handle);
    if ($body === false || $status < 300 || $status >= 600) {
        throw new RuntimeException('Browser proxy request failed: ' . $error);
    }
    http_response_code($status);
    foreach ($responseHeaders['location'] ?? [] as $value) {
        if (str_starts_with($value, 'https://')) header('Location: ' . $value);
    }
    foreach ($responseHeaders['set-cookie'] ?? [] as $value) {
        if (str_starts_with($value, '__Secure-sickwallet-session=')) header('Set-Cookie: ' . $value, false);
    }
